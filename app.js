...
const liftConf={
  squat:[ [55,10,16],[85,10,20],[125,10,10] ],
  bench:[ [70,7.5,16],[82.5,5,20],[105,5,10] ],
  dl:   [ [60,15,16],[105,12.5,20],[155,10,10] ],
  pullup:[ [10,5,16],[27.5,2.5,20],[35,2.5,10] ],
  front:[ [42.5,5,16],[57.5,5,20],[77.5,5,10] ],
  rdl:  [ [45,7.5,16],[67.5,5,20],[92.5,5,10] ],
  row:  [ [42.5,5,16],[62.5,5,20],[87.5,5,10] ],
  pp:   [ [32.5,5,16],[47.5,5,20],[72.5,5,10] ],
  pushpress:[ [30,5,16],[50,2.5,20],[60,2.5,10] ],
  hp:   [ [20,12.5,16],[65,7.5,20],[95,7.5,10] ],
  clean:[ [35,5,16],[70,5,20],[92.5,5,10] ],
};
const LOAD={};
...
function phaseGoalNote(wk){
  const p=phaseOf(wk), b=wkInBlock(wk);
  const goals={1:'减脂期：热量按久坐基准-300-400，蛋白 2.0g/kg；动作模式打底（蹲/拉/推/髋铰链/爆发）',2:'增肌期：热量 +250，蛋白 1.8g/kg；每周加重，高翻逐步加量',3:'塑形期：热量 -300，蛋白 2.0g/kg；力量与爆发保底'};
...
function buildDay(wk,idx){
  ...
  const mainPow=isCleanWeek(wk)
    ? [cleanName(wk),cleanSets(wk),LOAD.clean[i]+'kg','爆发主项：髋膝蹬伸→耸肩→接杠；接住后控制下放（不砸杠）',true,'clean']
    : [hpName(wk),hpSets(wk),LOAD.hp[i]+'kg','爆发主项，速度优先；先分解再连贯',true,'highpull'];
  const lists=[
    [
      ['动态热身（高抬腿/开合跳/弓步走/髋绕环/踝膝活动）','10分钟','自重','提高体温与活动度',false,null],
      ['深蹲（主项，空杆递增2-3组）',mr,LOAD.squat[i]+'kg','留1-2次余量；速度掉20%停组',true,'squat'],
      ['硬拉（主项）',dlReps(wk),LOAD.dl[i]+'kg','髋铰链、杠铃贴身、脊柱中立'+(wk<=3?'；录视频自查':''),true,'dl'],
    ...
    [
      ...
      ['卧推（主项）',mr,LOAD.bench[i]+'kg','留1-2次余量（不做到力竭）；保持杠速',true,'bench'],
...
function renderPlan(){
  ...
  const alloc=[
    ['减脂·技术期','W1-16','4练/周','主项 4×4-6（减载 3×5）','中容量：每肌群 10-12 组/周','练后拉伸 + 休息日轻跑 1-2 次','1900 / 1650 大卡 · 蛋白 150g'],
    ['增肌·力量期','W17-36','4练/周','主项 5×5 → 4×3（减载 3×5）','高容量：每肌群 13-15 组/周','跳跃/冲刺保持，每周 1 次','2500 / 2250 大卡 · 蛋白 145g'],
    ['塑形·冲刺期','W37-46','4练/周','主项 3×3-4，强度优先','低容量：每肌群 8-10 组/周','力量保底 + 体能维持','1850 / 1650 大卡 · 蛋白 150g'],
  ];
...
const EVIDENCE=[
  {cat:'主项与辅项关系（为什么这样排）',list:[
    ...
    {t:'辅项承担增肌容量与补弱',s:'Schoenfeld 等 2017 · J Sports Sci（容量荟萃）',c:'每肌群每周 10+ 组有效容量是增肌关键；主项负责力量，辅项把容量补齐并针对主项弱点。',u:'每肌群 10-15 组/周=主项+辅项合计；保加利亚/RDL 补深蹲硬拉，侧平举/碎颅者补卧推。'},
  ...
  {cat:'力量与增肌（肌肉大且好用）',list:[
    ...
    {t:'每周训练容量与增肌剂量关系',s:'Schoenfeld 等 2017 · J Sports Sci（荟萃）',c:'每肌群每周 10+ 组有效训练，增肌收益随容量上升。',u:'四天分化：胸/背/腿/肩每周 2 次刺激，主项+辅助合计每肌群 10-15 组。'},
  ...
  {cat:'恢复与睡眠',list:[
    ...
    {t:'减载与表现提升',s:'Bosquet 等 2007 · Sports Med（荟萃）',c:'赛前/阶段末减载 1-2 周可提升表现 2-6%。',u:'每 4 周小减载 + 每阶段末一周过渡，主项降重 5kg、次数下调。'},
  ]},
