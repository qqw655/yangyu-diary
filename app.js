...
const DAY_ORDER = [1,2,3,5]; // 周一/周二/周三/周五
const DAY_NAMES = ['周一·推（胸·肩）','周二·拉（背·臂）','周三·蹲（下肢）','周五·美型补强'];
...
/* 阶段目标总览 */
const PHASE_TARGETS=[
  ['减脂·技术期','W1-16','减脂 + 推拉蹲动作打底 + 美型弱区启动（上胸/肩/背阔）','65-67kg / 12-14%','深蹲95-100 · 硬拉125-130 · 卧推90-95 · 引体+25-30 · 肩围110-115'],
  ['增肌·力量期','W17-36','推拉蹲力量增长 + 美型弱区加量（肩宽/背宽/上胸/臂）','71-73kg / 13-15%','深蹲125-135 · 硬拉155-165 · 卧推100-107 · 引体+35-40 · 肩围115-120'],
  ['塑形·冲刺期','W37-46','减脂显形 + 美型冲刺 + 力量保底','70-72kg / 12-13%','深蹲130-140 · 硬拉165-170 · 卧推107-112 · 引体+40 · 肩围118-122'],
];
...
function phaseGoalNote(wk){
  const p=phaseOf(wk), b=wkInBlock(wk);
  const goals={1:'减脂期：热量按久坐基准-300-400，蛋白 2.0g/kg；动作模式打底（蹲/拉/推/髋铰链/爆发）',2:'增肌期：热量 +250，蛋白 2.0g/kg；每周加重，爆发动作渐进',3:'塑形期：热量 -300，蛋白 2.0g/kg；力量与爆发保底'};
...
function restOf(it){
  ...
  if(n.includes('跳')||n.includes('深蹲')||n.includes('摆动')||n.includes('摆荡')||n.includes('借力推'))return '休3-5分钟';
  if(n.includes('罗马尼亚')||n.includes('实力推')||n.includes('侧平举'))return '休2-3分钟';
...
const DIET_NOTE='热量按最近记录体重 + 久坐消耗自动换算（不含训练）。训练日本身已含练前加餐的能量；当天若额外运动（散步/快走/跑步），每 30 分钟中高强度活动可自行补约 100-150kcal 碳水。';
...
function todayDay(){
  ...
  if(!(dow===1||dow===2||dow===3||dow===5))return null;
...
const DAY_DURATION={0:{base:85,deload:75},1:{base:75,deload:65},2:{base:75,deload:65},3:{base:85,deload:75}};
const dayDuration=(wk,idx)=>wkInBlock(wk)===4?DAY_DURATION[idx].deload:DAY_DURATION[idx].base;

/* ================= 课表构建（含肩宽专项 + 全面发展） ================= */
function buildDay(wk,idx){
  const i=wk-1,p=phaseOf(wk),mr=mainReps(wk);
  ...
  const lists=[
    ...
    [
      ...
      ['卧推（主项）',mr,LOAD.bench[i]+'kg','主项：'+rir+'；保持杠速；你当前参考 75kg×4-5（RIR 0-1）'+cond,true,'bench'],
      ['借力推（爆发）',ppSets(wk),LOAD.pushpress[i]+'kg','髋膝蹬伸→推举，速度优先；3次轻松→+2.5kg',false,'pushpress'],
      ['上斜哑铃卧推（上胸·美型重点）','3×8-10',acc('哑铃 10-12.5kg×2','哑铃 12.5-15kg×2','哑铃 15-17.5kg×2'),'上胸是美型第一弱区；RIR 2-3，严格控制，顶端不锁死',false,null],
      ['哑铃侧平举（中束·肩宽）',acc('4×12-15','5×12-15','4×15'),acc('10kg×2','12.5kg×2','15kg×2'),'美型重点·肩宽；RIR 2-3，不借力不甩',false,null],
      ['双杠臂屈伸','3×8-10',acc('自重 ~ +5kg','+10 ~ +15kg','+17.5 ~ +22.5kg'),'辅项·胸下束+三头；RIR 2-3',false,null],
      ['仰卧臂屈伸（碎颅者）','3×10-12',acc('10kg','12.5kg','15→20kg（W杆+1.25kg片）'),'辅项·三头长头（臂围）；RIR 2-3，手肘朝前慢放',false,null],
    ...
    [
      ...
      ['负重引体（宽握，主项）',mr,'+'+LOAD.pullup[i]+'kg','全程控制，不摆荡；宽握练背阔宽度；主项：'+rir,true,'pullup'],
      ['杠铃划船','4×6-8',LOAD.row[i]+'kg','辅项·背部厚度；你当前参考 50kg 做组（RIR 2）；脊柱中立',false,null],
      ['弹力带面拉（后束+肩袖）','3×15-20','弹力带','辅项·肩胛健康+后束（防圆肩）；RIR 2-3',false,null],
    ...
    [
      ...
      ['站姿提踵','3×12-15',acc('负重 20-30kg','负重 25-35kg','负重 30-40kg'),'辅项·小腿；RIR 2-3',false,null],
      ['悬垂举腿（控制版）','3×10-12','自重','腹直肌·慢放2-3秒不摆荡；轻松×12→负重',false,null],
      ['静态拉伸（髋屈肌/腘绳肌/胸椎）','5分钟','—','收尾放松',false,null],
    ],
    /* 周五 · 美型补强（弱区+爆发）【中】 */
    [
      ['动态热身（全身+肩带激活）','8分钟','自重/弹力带','激活肩袖、背阔与髋',false,null],
      ['实力推（主项，空杆递增）',mr,LOAD.pp[i]+'kg','主项：'+rir+'；你当前参考 40kg×3-4；速度掉20%停组'+cond,true,'pp'],
      ['上斜哑铃卧推（上胸·第二次刺激）','3×10-12',acc('哑铃 10-12.5kg×2','哑铃 12.5-15kg×2','哑铃 15kg×2'),'美型重点·上胸；RIR 2-3，严格控制',false,null],
      ['哑铃侧平举（中束·第二次刺激）','3×15-20',acc('哑铃 7.5-10kg×2','哑铃 10-12.5kg×2','哑铃 12.5kg×2'),'美型重点·中束；RIR 2-3，不借力',false,null],
      ['俯身哑铃反向飞鸟（后束·弱区）','3×12-15',acc('哑铃 5-7.5kg×2','哑铃 7.5-10kg×2','哑铃 10kg×2'),'美型重点·后束；RIR 2-3，肩胛放松只动肩关节',false,null],
      ['宽握引体（背阔·弱区·轻容量）','3×8-10','自重或轻负重','美型重点·背阔宽度；RIR 2-3，不摆荡',false,null],
      ['跳深 / 连续跳（下肢弹性）','4×3-5','30-40cm箱','落地缓冲，质量优先',false,null],
      ['摸高 / 单脚跳（踝弹性）','3×3-5','自重','爆发动作，每次全速',false,null],
      ['平板支撑 + 死虫','各3组','自重','核心稳定+控制',false,null],
      ['静态拉伸（胸/肩/背/髋/腘绳）','5分钟','—','收尾',false,null],
    ],
  ];
...
function renderToday(){
  ...
  const isTrain=(dow===1||dow===2||dow===3||dow===5);
  const p=phaseOf(wk);
  ...
  if(isTrain){
    ...
    wkEl.innerHTML='<div class="rest-note big">💤 好好休息<br>恢复也是训练的一部分：散步或快走 20-30 分钟、拉伸、睡前泡沫轴。状态好就动一动，状态差直接休息。<br>'+esc(phaseGoalNote(wk))+'</div>';
  }
...
function renderPlan(){
  ...
  const alloc=[
    ['减脂·技术期','W1-16','4练/周：推·拉·蹲·美型补强','主项 5×5（减载 3×5）','中容量：每肌群约 8-12 组/周','周五：弱区补强+爆发（跳深/摸高）','按体重自动换算 · 蛋白 ~2.1g/kg'],
    ['增肌·力量期','W17-36','4练/周：推·拉·蹲·美型补强','主项 5×5（减载 3×5）','高容量：每肌群约 12-18 组/周','周五：弱区补强+爆发（跳深/摸高）','按体重自动换算 · 蛋白 ~2.1g/kg'],
    ['塑形·冲刺期','W37-46','4练/周：推·拉·蹲·美型补强','主项 3×3-4，强度优先','低容量：每肌群约 8-12 组/周','周五：弱区补强+爆发（跳深/摸高）','按体重自动换算 · 蛋白 ~2.1g/kg'],
  ];
...
const EVIDENCE=[
  ...
  {cat:'力量与增肌（肌肉大且好用）',list:[
    ...
    {t:'每周训练容量与增肌剂量关系',s:'Schoenfeld 等 2017 · J Sports Sci（荟萃）',c:'每肌群每周 10+ 组有效训练，增肌收益随容量上升，但新手 6-8 组也有效。',u:'四天分化：胸/背/肩=主项日+周五美型补强（每周 2 次刺激），腿=周三重主项+周五跳跃；每肌群约 8-14 组/周。'},
    {t:'组间休息时长',s:'Grgic 等 2018 · Sports Med（荟萃）',c:'复合大重量动作组间休 3-5 分钟，力量收益优于短间歇。',u:'主项与跳跃类统一休 3-5 分钟，辅助 60-90 秒，已写进每组的休息提示。'},
    {t:'自然训练者的频率与容量',s:'Schoenfeld 等 2016 · J Sports Sci（频率荟萃）；Helms 等 2014 · JISSN',c:'自然训练者建议每肌群每周 2 次左右刺激、容量渐进，比单次练到瘫更有效。',u:'弱区（上胸/中束/后束/背阔）在主项日之外，周五美型日补第二次刺激；上胸/侧平举/反向飞鸟都按一周两次安排，是美观优先的核心。'},
  ]},
  {cat:'爆发力与运动表现（好用）',list:[
    {t:'力量是爆发力的基础',s:'Suchomel, Nimphius & Stone 2016 · Sports Med',c:'最大力量水平决定爆发力的天花板；先涨力量，爆发动作才有效。',u:'前 16 周（减脂·技术期）以深蹲硬拉打底，跳跃只做技术，不冲重量。'},
    {t:'爆发力需要重负荷+快速动作结合',s:'Cormie, McGuigan & Newton 2011 · Sports Med',c:'纯重负荷或纯轻快速都无法最大化爆发力，两者结合最优。',u:'主项力量日（周一推/周二拉/周三蹲）与周五美型日的跳跃分开安排，形成"重-快"组合。'},
    {t:'增强式训练提升弹跳',s:'Markovic 2007 · Br J Sports Med（荟萃）',c:'跳深、跳箱等增强式训练是提升垂直弹跳的有效手段。',u:'跳跃刺激集中在周五美型日（跳深/连续跳 + 摸高），速度优先、落地缓冲，与周三腿部力量日分日安排。'},
    {t:'增强式跳跃是可靠的爆发路径',s:'Tricoli 等 2005 · J Strength Cond Res',c:'短期内增强式跳跃能显著提升垂直跳与冲刺表现，不依赖复杂举重动作。',u:'周五美型日以跳深/连续跳 + 摸高承担爆发刺激，力量日同步打底，下肢爆发稳步提升。'},
  ]},
...
function copyLog(){
  ...
  const isTrain=(dow===1||dow===2||dow===3||dow===5),p=phaseOf(wk),tag=dietTag(wk,isTrain),m=macroFor(p,tag);
